...
public class BlueBlimpSprite implements DisplayableSprite {
	...
	public BlueBlimpSprite(double centerX, double centerY) {
		...
		if (images == null) {
			try {
				images = new Image[8];
				for (int i = 0; i < 8; i++) {
					String path = String.format("res/blueBlimp.png", i);
	...
	public void update(Universe universe, KeyboardInput keyboard, long actual_delta_time) {
		...
		if ( (this.centerX < -200) && direction == Direction.LEFT) {
			//at left edge, move down
			direction = Direction.DOWN;
		}
		if ((this.centerX > 200) && direction == Direction.RIGHT) {
